...
@Component({
  selector: 'app-preguntas-quiz-diagnostico',
  templateUrl: './preguntas-quiz-diagnostico.page.html',
  styleUrls: ['./preguntas-quiz-diagnostico.page.scss'],
})
export class PreguntasQuizDiagnosticoPage implements OnInit {
  @Input() answer: string;
...
//Preguntas
  ...
  constructor(public configuracion:ConfiguracionService,private route: ActivatedRoute, public navCtrl: NavController,private router: Router) {
    ...
    this.route.paramMap.subscribe(params => {
      this.setPreguntaID(+params.get('PreguntaId'));  // get the question ID and store it
      this.CargarPregunta();
  ...
  NavegarSiguientePregunta(): void {
    ...
    this.Preguntas.SeleccionRespuestaPrincipal = '';
    this.TieneRespuesta=false;
    this.navCtrl.navigateRoot(['/preguntas-quiz-diagnostico', this.getPreguntaID() + 1]).then;
